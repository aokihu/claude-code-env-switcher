...
- **THEN** the tool processes the provider and model arguments
- **AND** the configuration is applied to subsequent operations

#### Scenario: User uses short flags
- **WHEN** user runs `router-switch -p anthropic -m claude-3`
- **THEN** the tool processes the provider and model arguments using short flags
- **AND** the configuration is applied to subsequent operations
